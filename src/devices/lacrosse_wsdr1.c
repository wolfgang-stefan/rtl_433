...
static int lacrosse_wsdr1_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    int raw_wind  = b[4] << 4 | ((b[5] & 0xf0) >> 4);
    int direction = ((b[5] & 0x0f) << 8) | b[6];
    int raw_rain1 = b[7] << 4 | ((b[8] & 0xf0) >> 4);
    int raw_rain2 = ((b[8] & 0x0f) << 8) | b[9];
    ...
    data_t *data = data_make(
            "model",            "",                 DATA_STRING, "LaCrosse-WSDR1",
            ...
            "wind_avg_km_h",    "Wind Speed",       DATA_FORMAT, "%.1f km/h", DATA_DOUBLE, wspeed_kmh,
